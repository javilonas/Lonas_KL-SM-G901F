...
static void ImmVibeSPI_vibtonz_en(bool en)
{
	switch (vibrator_drvdata.motor_ic) {
#if defined(CONFIG_MOTOR_DRV_MAX77828)
	case MAXIM_MAX77828:
		max77828_vibtonz_en(en);
		break;
#endif
#if defined(CONFIG_MOTOR_DRV_MAX77804K)
	case MAXIM_MAX77804K:
		max77804k_vibtonz_en(en);
		break;
#endif
#if defined(CONFIG_MOTOR_DRV_MAX77843)
	case MAXIM_MAX77843:
		max77843_vibtonz_en(en);
		break;
#endif
	default:
		break;
	...
	if (!g_bampenabled) {
		...
		if (vibrator_drvdata.vib_model == HAPTIC_PWM) {
			if(vibrator_drvdata.is_pmic_vib_pwm){ //PMIC PWM
				...
				pr_err("[VIB] %s : 2\n", __func__);
				rc = gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.vib_pwm_gpio,\
	...
	*/
	...
	if (vibrator_drvdata.vib_model == HAPTIC_PWM) {
		if (vibrator_drvdata.is_pmic_vib_pwm) { //PMIC PWM
			...
			gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.vib_pwm_gpio,
			3, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
			GPIO_CFG_ENABLE);
		}
	}
						...
						int8_t *pForceOutputBuffer)
{
	...
	/* pr_info("%s: nOutputSignalBitDepth : %d\n", __func__, nOutputSignalBitDepth); */
