...
void si_set_cbus_mode_leds(enum cbus_mode_e mode_sel)
{
	...
	case CM_eCBUS_S:
	case CM_eCBUS_D:
		MHL_TX_DBG_ERR("CBUS MODE: MHL3\n");
		set_pin(X02_USB_LED15_AMBER, 1);
		set_pin(X02_USB_LED15_GREEN, 0);
		set_pin(LED_USB_MODE, 1);
		break;
	default:
		MHL_TX_DBG_ERR("CBUS MODE: %02X\n", mode_sel);
		break;
	}
}
#endif
...
#ifdef CONFIG_MHL3_SEC_FEATURE
		mhl_tx_write_reg(hw_context, REG_MHL_COC_CTL1, dev_context->pdata->coc_ctl1);
#else
		/* Fix CTS 3.3.2.3 0xBC -> 0xBD */
...
#ifdef CONFIG_MHL3_SEC_FEATURE
		/* Pre-emphasis tuning */
		if (dev_context->pdata->pre_emphasis_set) {
			mhl_tx_write_reg(hw_context, TX_PAGE_3 | 0x50,
					dev_context->pdata->pre_emphasis_set);
			mhl_tx_write_reg(hw_context, TX_PAGE_3 | 0x52,
					dev_context->pdata->pre_emphasis_value);
		}
#endif
				...
				const char *function, int line_num)
{
	if (debug_level >= DBG_MSG_LEVEL_WARN) {
		...
		for (i = 0, c = (unsigned char *)&vsif; i < sizeof(vsif);
			++i, ++c) {
			MHL_TX_DBG_INFO(KERN_CONT " %02x", *c);
		}
		pr_info("\n");
...
#define ENABLE_FP	VAL_VID_OVRRD_3DCONV_EN_FRAME_PACK
	...
	MHL_TX_DBG_INFO("input_quant_range1 = [%02x]\n", input_quant_range );
#ifdef CONFIG_MHL3_SEC_FEATURE
	...
	MHL_TX_DBG_ERR("input_clr_spc = %02X infoData[0]:%02X\n",
		input_clr_spc,
		hw_context->current_avi_info_frame.payLoad.hwPayLoad.
		namedIfData.ifData_u.infoFrameData[0]);

	print_hex_dump(KERN_INFO, "INFO data = ",
			DUMP_PREFIX_OFFSET, 13, 1,
			hw_context->current_avi_info_frame.payLoad.hwPayLoad.
			namedIfData.ifData_u.infoFrameData
			, 13, false);
...
/* begin BZ 32674 ( */
			...
			if (aqr_default != input_quant_range) {
				...
				MHL_TX_DBG_INFO("input_quant_range2 = [%02x]\n", input_quant_range );
#if 0
...
/* end BZ 32674 ) */
...
#ifdef CONFIG_MHL3_SEC_FEATURE
		//On Samsung platform, it forces to send limited ranage video regardless of HDMI input.
		output_quant_range = 0x02;
#else
	...
	hw_context->outgoingAviPayLoad.ifData[3] &= 0x03; /* Clear PB3[2-7] */
#endif
	...
	uint16_t length)
{
	...
	MHL_TX_DBG_INFO("sets upstream HPD high\n");
...
{
	mhl_tx_write_reg(hw_context, REG_HDCP1X_LB_BIST,
		BIT_HDCP1X_LB_BIST_HDCP1X_LB_BIST_EN);
				    ...
				    uint8_t *edid, uint16_t length)
{
	...
	if (dev_context->pdata->hdmi_mhl_ops) {
		...
		hdmi_mhl_ops->set_upstream_hpd(dev_context->pdata->hdmi_pdev, 1);
	}
	msleep(50);
#endif
...
static void unmute_video(struct drv_hw_context *hw_context)
{
	...
	MHL_TX_DBG_INFO("No HDCP\n");
#else
	if(0x01 && mhl_tx_read_reg(hw_context, REG_TPI_SC))
		MHL_TX_DBG_ERR(" HDCP1.3 Authentication Done\n");
#endif

	MHL_TX_DBG_ERR("AV unmuted\n");
...
#if defined(CONFIG_SEC_MHL_AP_HDCP_PART1)
	platform_ap_hdmi_hdcp_auth(dev_context->pdata);
#endif
	MHL_TX_DBG_INFO("Start HDCP Authentication\n");
...
static int hdcp_isr(struct drv_hw_context *hw_context, uint8_t intr_status)
{
	...
	MHL_TX_DBG_ERR("R3D= %02x R29= %02x\n", intr_status, query_data);
	...
	if (BIT_TPI_INTR_ST0_READ_BKSV_BCAPS_DONE_STAT & intr_status) {
		...
		*/
		...
		MHL_TX_DBG_ERR("BKSV ERROR-Restart HDCP !!!\n");
		start_hdcp(hw_context);
		...
		case VAL_TPI_COPP_LINK_STATUS_LINK_LOST:
			...
			MHL_TX_DBG_ERR(" LINK LOST - Restart HDCP!!!\n");
			start_hdcp(hw_context);
			break;
		case VAL_TPI_COPP_LINK_STATUS_RENEGOTIATION_REQ:
			MHL_TX_DBG_ERR("LINK_STATUS_RENEGOTIATION\n");
			MHL_TX_DBG_ERR("tpi BSTATUS2: 0x%x\n",
					mhl_tx_read_reg(hw_context,
							REG_TPI_BSTATUS2)
			    );
			/*
		...
		case VAL_TPI_COPP_LINK_STATUS_LINK_SUSPENDED:
			...
			MHL_TX_DBG_ERR("LINK suspended - Restart HDCP!!!\n");
			start_hdcp(hw_context);
			break;
		}
		...
		case (VAL_TPI_COPP_GPROT_NONE | VAL_TPI_COPP_LPROT_NONE):
			...
			MHL_TX_DBG_ERR("GPROT_NONE - Retart HDCP!!!\n");
			start_hdcp(hw_context);
			break;
			   ...
			   bool do_interrupt_clear)
{
	...
	stop_video(hw_context);
	MHL_TX_DBG_ERR("STOP_VIDEO DONE\n");
	...
	struct interrupt_info *intr_info)
{
	...
	MHL_TX_DBG_INFO("%s got INTR COC_STAT_0:0x%02x%s\n",
			ANSI_ESC_GREEN_TEXT,
			mhl_tx_read_reg(hw_context, REG_COC_STAT_0),
			ANSI_ESC_RESET_TEXT);

	MHL_TX_DBG_INFO("%s div_ctl_main:0x%02x hdcp2x_tp1:0x%02x%s\n",
			ANSI_ESC_GREEN_TEXT,
...
int si_mhl_tx_chip_initialize(struct drv_hw_context *hw_context)
{
	...
	MHL_TX_DBG_INFO(": ELEC_EYE_TESTONLY\n");
	...
	if (ret_val > 0)
	{
	  ...
	  MHL_TX_DBG_INFO(": chip_init success! - ELEC_EYE_TESTONLY\n");
	  return status;
...
int si_mhl_tx_chip_initialize(struct drv_hw_context *hw_context)
{
...
#endif
	ret_val = get_device_id(hw_context);
	MHL_TX_DBG_INFO(": Device check =%04x\n", ret_val);
	if (ret_val > 0) {
		struct mhl_dev_context *dev_context =
			container_of((void *)hw_context,
				struct mhl_dev_context, drv_context);
	...
	MHL_TX_DBG_INFO(": chip_init success!\n");
	return status;
}
