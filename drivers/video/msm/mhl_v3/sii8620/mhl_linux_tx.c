...

{
	...
	if (event) {
		MHL_TX_DBG_INFO(":(%d) MHL-Connection(TESTONLY)*****\n", (int)event);
		clk_prepare_enable(mhl_clk);
		dev_context->pdata->charger_mhl_cb(true, 0x03);
		dev_context->pdata->power(true);
		si_mhl_tx_initialize(dev_context);
		MHL_TX_DBG_INFO(": mhl3-elc.defualt reg set++ \n");
		...
		for (i = 0; i < dev_context->pdata->m_cnt; i++) {
			...
			MHL_TX_DBG_INFO(":0x%02x x 0x%02x = 0x%02x \n", dev_context->pdata->m_page[i],
					dev_context->pdata->m_offset[i], dev_context->pdata->m_value[i]);
		}
	} else {
		MHL_TX_DBG_INFO(":(%d) MHL-Disconnection(TESTONLY)*****\n", (int)event);
...
#endif /* #ifdef CONFIG_TESTONLY_SYSFS_SW_REG_TUNING */

#ifdef CONFIG_EXTCON
int sii8620_detection_callback(unsigned long event)
{
	struct mhl_dev_context *dev_context = dev_get_drvdata(sii8620_dev);
	MHL_TX_DBG_ERR(": sii8620_detection_callback : event=%lu*****\n",
			event);

	if (dev_context->detection_state == event)
	{
		MHL_TX_DBG_ERR(": already %s\n", event ? "attached" : "detached");
		return 0;
	}
	...
	if (event) {
		...
		dev_context->pdata->ap_hdcp_success = false;
#endif
	} else {
		if (dev_context->pdata->hdmi_mhl_ops) {
			struct msm_hdmi_mhl_ops *hdmi_mhl_ops =	dev_context->pdata->hdmi_mhl_ops;
			hdmi_mhl_ops->set_upstream_hpd(dev_context->pdata->hdmi_pdev, 0);
		}
		MHL_TX_DBG_ERR(": MHL-Disconnection\n");
		...
		if (dev_context->mhl_event_switch.state != MHL_EVENT_CLR) {
			MHL_TX_DBG_INFO(": MHL switch event sent : %d\n", MHL_EVENT_CLR);
					...
					unsigned long event, void *ptr)
{
	struct mhl_dev_context *dev_context = container_of(this, struct mhl_dev_context,
						mhl_nb);
	if (dev_context->muic_state == event)
	{
		MHL_TX_DBG_ERR(": already %s\n", event ? "attached" : "detached");
		return 0;
	}
...
#else
	if (event) {
		MHL_TX_DBG_ERR(":detection started\n");
		dev_context->pdata->power(true);
		si_mhl_tx_initialize(dev_context);
		enable_irq(dev_context->drv_info->irq);
	} else {
		MHL_TX_DBG_ERR(":disconnection\n");
		disable_irq(dev_context->drv_info->irq);
		dev_context->pdata->charger_mhl_cb(false, -1);
		dev_context->pdata->power(false);
		if (dev_context->mhl_event_switch.state != MHL_EVENT_CLR) {
			MHL_TX_DBG_ERR(": MHL switch event sent : %d\n", MHL_EVENT_CLR);
			switch_set_state(&dev_context->mhl_event_switch, MHL_EVENT_CLR);
		}
	}
#endif /* #ifdef CONFIG_TESTONLY_SYSFS_SW_REG_TUNING */
...
uint8_t mhl_get_version(void)
{
	...
	struct mhl_dev_context *dev_context = dev_get_drvdata(parent_dev);
...
int mhl_get_video_linkmode(void)
{
	...
	return dev_context->dev_cap_cache.mdc.vid_link_mode;
}

int mhl_get_link_speed(void)
{
	struct device *parent_dev = &spi_dev->dev;
	struct mhl_dev_context *dev_context = dev_get_drvdata(parent_dev);
		...
		char *buf)
{
	...
	MHL_TX_DBG_ERR(": get Device_ID, spi read : ret_val = %04x\n",
			ret_val);
read_failed:
	pdata->power(0);
		...
		struct class_attribute *attr, char *buf)
{
	...
	size = snprintf(buf, 10,"%d", sii8620->pdata->monitor_cmd.a);
	return size;
}

static CLASS_ATTR(hdcp_status, 0444,
		sii8620_hdcp_status, NULL);
...
static ssize_t sii8620_test_v2_test_show(struct class *dev,
		struct class_attribute *attr, char *buf)
{
	MHL_TX_DBG_ERR("force_ocbus_for_ects = %d mhl version = %d\n",
		...
		const char *buf, size_t size)
{
	...
	MHL_TX_DBG_ERR("force_ocbus_for_ects = %d\n", force_ocbus_for_ects);
	return size;
}
		...
		const char *buf, size_t size)
{
	...
	if (numbers[0] == 3)
	{
		MHL_TX_DBG_INFO(" write page:%02x offset:%02x data:%02x\n",
				numbers[1], numbers[2], numbers[3]);
		mhl_tx_write_reg_elc(sii8620, numbers[1], numbers[2], numbers[3]);
	} else if (numbers[0] == 2) {
		MHL_TX_DBG_INFO(" read page:0x%02x offset:0x%02x\n", numbers[1], numbers[2]);
...
#ifdef CONFIG_MHL3_SEC_FEATURE
int mhl_tx_init(struct mhl_drv_info const *drv_info, struct device *parent_dev, struct sii8620_platform_data *pr_data)
#else
...
#endif
{
	...
	MHL_TX_DBG_INFO(": MHL transmitter init start\n");
	if (drv_info == NULL || parent_dev == NULL) {
		MHL_TX_DBG_ERR(": Null parameter passed to mhl_tx_init\n");
		return -EINVAL;
	}
	...
	if (mhl_class == NULL) {
		...
		if (IS_ERR(mhl_class)) {
			...
			MHL_TX_DBG_INFO(": class_create failed %d\n", ret);
			goto err_exit;
		}
		...
		if (ret) {
			MHL_TX_DBG_INFO(": register_chrdev %s failed, error code: %d\n",
				MHL_DRIVER_NAME, ret);
			goto free_class;
		}
		...
		if (ret) {
			MHL_TX_DBG_INFO(": cdev_add %s failed %d\n", MHL_DRIVER_NAME, ret);
			goto free_chrdev;
		}
#ifdef CONFIG_MHL3_SEC_FEATURE
#ifdef CONFIG_SS_FACTORY
		...
		if (ret)
			MHL_TX_DBG_ERR(": failed to create device file in sysfs entries!\n");
#endif
#ifdef CONFIG_MHL_CONFIG_TOOLS
		ret = class_create_file(mhl_class, &class_attr_swing_v2);
		if (ret)
			MHL_TX_DBG_ERR(": failed to create swing_2 sysfs file\n");

		ret = class_create_file(mhl_class, &class_attr_swing_v3);
		if (ret)
			MHL_TX_DBG_ERR(": failed to create swing_3 sysfs file\n");

		ret = class_create_file(mhl_class, &class_attr_timing);
		if (ret)
			MHL_TX_DBG_ERR(": failed to create timing sysfs file\n");
		ret = class_create_file(mhl_class, &class_attr_test_case);
		if (ret)
			MHL_TX_DBG_ERR(": failed to create test_case sysfs file\n",
					__func__, __LINE__);
		...
		if (ret)
			MHL_TX_DBG_ERR(": failed to create reg_tuning sysfs file\n");

		ret = class_create_file(mhl_class, &class_attr_reg_set);
		if (ret)
			MHL_TX_DBG_ERR(": failed to create reg_set sysfs file\n",
					__func__, __LINE__);
#endif
	...
	if (IS_ERR(dev_context->mhl_dev)) {
		...
		MHL_TX_DBG_INFO(": device_create failed %s %d\n",MHL_DEVICE_NAME, ret);
		goto free_cdev;
	}
	{
		int status = 0;
	...
	dev_context->edid_parser_context =
		si_edid_create_context(dev_context,
			(struct drv_hw_context *)&dev_context->drv_context);
	MHL_TX_DBG_INFO(": Initialize EDID parser module\n");
	...
	mhl_clk = clk_get(sii8620_dev, "xtal_mhl");
	if (IS_ERR(mhl_clk))
		dev_err(sii8620_dev, "failed to get clock 'xtal_mhl'\n");
